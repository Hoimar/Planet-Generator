...
onready var orgPivotTransform: Transform = cameraPivot.get_transform()
onready var orgCameraRotation: Vector3 = camera.rotation
...
func _input(event):
	if event is InputEventMouseMotion:
		mouseSpeed = event.relative * PGGlobals.MOUSE_SENSITIVITY
